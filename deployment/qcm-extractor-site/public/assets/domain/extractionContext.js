function compactRegion(region, localPage) {
    return {
...
export function createExtractionContext(batch, documentMap) {
    const localPageByOriginal = new Map(batch.pageMap.map((entry) => [entry.originalPage, entry.localPage]));
    const segmentMap = new Map(documentMap.question_segments.map((segment) => [segment.temporary_id, segment]));
    return {
        batch_id: batch.id,
        ...
        segment_page_map: Object.fromEntries(batch.segmentReferences.map((reference) => [reference.segmentId, reference.sourcePages])),
