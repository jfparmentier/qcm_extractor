...
function answerFraction(question: ReviewExportDocument["questions"][number], choiceId: string): string {
  if (!question.correct_choice_ids.includes(choiceId)) return "-100";
...
export async function createMoodleXml(
  value: ReviewExportDocument,
  generatedAssets: Readonly<Record<string, GeneratedIllustrationAsset>>
): Promise<string> {
  const assetsByFileName = new Map(
    Object.values(generatedAssets).map((asset) => [asset.fileName, asset] as const)
  );
