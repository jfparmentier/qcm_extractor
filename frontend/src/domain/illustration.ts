...
export interface IllustrationCandidate {
  ...
  readonly regionOrigin: PageRegionOrigin;
  readonly altText: string;
  readonly insertionToken: string;
  readonly statementContainsToken: boolean;
  readonly fileName: string;
  readonly warnings: readonly string[];
}

export interface IllustrationPlan {
  readonly candidates: readonly IllustrationCandidate[];
...
export interface GeneratedIllustrationAsset extends IllustrationCandidate {
  readonly blob: Blob;
  ...
  readonly generationWarnings: readonly string[];
}

export interface IllustrationGenerationProgress {
...
export interface IllustrationGenerationState {
  readonly status: IllustrationGenerationStatus;
  readonly assets: Readonly<Record<string, GeneratedIllustrationAsset>>;
  readonly errors: Readonly<Record<string, string>>;
  readonly progress: IllustrationGenerationProgress | null;
  readonly startedAt: number | null;
}

export const INITIAL_ILLUSTRATION_GENERATION_STATE: IllustrationGenerationState = {
  status: "idle",
  assets: {},
  errors: {},
  progress: null,
...
function bboxDistance(first: NormalizedBoundingBox, second: NormalizedBoundingBox): number {
...
function selectMetadata(
...
): { readonly image: ExtractedImage; readonly index: number } | null {
  ...
  const best = ranked[0];
  if (best === undefined || best.score >= 100) return null;
...
function candidateFingerprint(candidate: IllustrationCandidate): string {
...
export function createIllustrationPlan(
  ...
  documentMap.question_segments.forEach((segment, segmentIndex) => {
    ...
    imageRegions.forEach((region, regionIndex) => {
      ...
      if (matched !== null) usedMetadataIndexes.add(matched.index);
    ...
    if (question !== null && usedMetadataIndexes.size < question.images.length) {
      warnings.push(
        `${segment.temporary_id} : ${question.images.length - usedMetadataIndexes.size} image(s) décrite(s) par le LLM ne correspondent à aucune zone d’image de la cartographie.`
      );
    }
  });
...
export function revokeIllustrationAssets(
  assets: Readonly<Record<string, GeneratedIllustrationAsset>>
): void {
