...
function dosTimestamp(date: Date): { readonly time: number; readonly date: number } {
...
function centralHeader(entry: PreparedZipEntry): Uint8Array {
...
export async function createZipBlob(entries: readonly ZipEntryInput[]): Promise<Blob> {
  if (entries.length === 0) throw new Error("L’archive ZIP ne contient aucun fichier.");
  if (entries.length > MAX_UINT16) throw new Error("L’archive contient trop de fichiers pour le format ZIP classique.");
  ...
  for (const input of entries) {
    ...
    if (names.has(name)) throw new Error(`Le fichier ${name} apparaît plusieurs fois dans l’archive.`);
    names.add(name);

    const nameBytes = new TextEncoder().encode(name);
    const data = await toBytes(input.data);
    if (nameBytes.byteLength > MAX_UINT16) throw new Error(`Le chemin ${name} est trop long.`);
    if (data.byteLength > MAX_UINT32) throw new Error(`Le fichier ${name} dépasse la limite de 4 Gio.`);
    if (localOffset > MAX_UINT32) throw new Error("L’archive dépasse la limite de 4 Gio.");
  ...
  if (centralOffset + centralSize > MAX_UINT32) throw new Error("L’archive dépasse la limite de 4 Gio.");
  return new Blob(
    [...localParts, ...centralParts, blobPart(endOfCentralDirectory(prepared.length, centralSize, centralOffset))],
